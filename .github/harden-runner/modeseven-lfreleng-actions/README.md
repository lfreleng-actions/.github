...
# modeseven-lfreleng-actions egress allow-list

`allow_list.txt` is the shared [harden-runner][hr] egress allow-list for
the `modeseven-lfreleng-actions` organisation. Workflows load it with
[harden-runner-block-action][block] and run harden-runner in `block`
mode, so harden-runner denies any host this file omits.

This organisation is a shadow/fork of `lfreleng-actions` used to test
actions and reusable workflows. The list mirrors the canonical
`lfreleng-actions` allow-list so forked workflows resolve an
org-specific allow-list (keyed on `github.repository_owner`) and behave
identically here.
