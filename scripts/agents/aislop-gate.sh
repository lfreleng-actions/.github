...
#
# Contract
#   aislop-gate.sh <base-ref>   gate the working tree against the merge
#                               base of HEAD and <base-ref>
#   aislop-gate.sh --staged     gate exactly what is staged
#   aislop-gate.sh --changes    gate the working tree against HEAD
#
#   The base-ref and --changes scopes read the working tree and include
#   untracked files, so the file list below adds them: aislop scores a
#   new file before it is staged, and the language check has to see the
#   same files aislop did or it cannot speak for them.
#
...
#   gated files include Python or Go, the report must show the lint
#   engine ran: a skipped engine means the binary aislop delegates to
#   is absent and the scan under-reports, and an absent status settles
#   nothing. Both fail.
#
# Exit status
...
# -z throughout. With core.quotePath at its default, git C-quotes any
# non-ASCII path, so a plain listing wraps the name in quotes and
# escapes its bytes, leaving the suffix the language check reads no
# longer at the end. NUL-delimited output is unquoted and unambiguous.
scope_files() {
  case "$1" in
    ...
    --changes)
      ...
      ;;
    *)
      git diff --name-only -z --diff-filter=ACMR "$1"
      git ls-files --others --exclude-standard -z
      ;;
  esac
}
...
# reports. Capture the report and judge it on its content. The status
# still matters in one direction, handled at the end: a clean report
# from a failed process is a scan that stopped early, not a pass.
status=0
"$@" > "$report" || status=$?

python3 - "$report" "$files" "$status" <<'PY'
import json
import os
import sys

report_path, files_path = sys.argv[1], sys.argv[2]
aislop_status = int(sys.argv[3])
...
with open(files_path, "rb") as fh:
    scoped = [os.fsdecode(name) for name in fh.read().split(b"\0") if name]

needs_engine = [f for f in scoped if f.endswith((".py", ".pyi", ".go"))]
engines = report.get("engines")
lint = engines.get("lint") if isinstance(engines, dict) else None
if needs_engine and (not isinstance(lint, dict) or "skipped" not in lint):
    fail(
        2,
        "report carries no lint-engine status while Python or Go files are"
        " in scope; the gate cannot establish that lint ran",
    )
if needs_engine and lint["skipped"] is not False:
    fail(
        3,
...
if aislop_status != 0:
    fail(
        2,
        f"aislop exited {aislop_status} but reported no findings;"
        " treating this as a scan that stopped early",
    )
