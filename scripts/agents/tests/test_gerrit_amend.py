...
def test_usage_rejects_empty_path_argument(
    run_script: Callable[..., Outcome], gerrit_repo: GitRepo
) -> None:
    """An empty path is one unreadable argument, not the no-file form."""
    head = gerrit_repo.head()
...
def test_refuses_valueless_change_id(
    run_script: Callable[..., Outcome], git_repo: GitRepo
) -> None:
    """A bare `Change-Id:` preserves nothing; refuse as if there were none."""
    git_repo.commit("Fix: valueless", trailers="Change-Id:")
    head = git_repo.head()
    outcome = run_script(SCRIPT, cwd=git_repo.path)
    assert outcome.returncode == EXIT_NO_CHANGE_ID
    assert git_repo.head() == head
...
def test_refuses_malformed_change_id(
    run_script: Callable[..., Outcome], git_repo: GitRepo
) -> None:
    """An ID that is not I plus 40 hex digits is not a Gerrit Change-Id."""
    git_repo.commit("Fix: malformed", trailers="Change-Id: Ideadbeef")
    head = git_repo.head()
    outcome = run_script(SCRIPT, cwd=git_repo.path)
    assert outcome.returncode == EXIT_NO_CHANGE_ID
    assert git_repo.head() == head
...
def test_refuses_when_head_has_a_malformed_second_change_id(
    run_script: Callable[..., Outcome], git_repo: GitRepo
) -> None:
    """A valid ID beside a bare `Change-Id:` is still two trailers."""
    git_repo.commit("Fix: bare second", trailers=f"{CHANGE_ID}\nChange-Id:")
    head = git_repo.head()
    outcome = run_script(SCRIPT, cwd=git_repo.path)
    assert outcome.returncode == EXIT_NO_CHANGE_ID
    assert git_repo.head() == head
...
def test_refuses_when_head_change_id_is_body_only(
    run_script: Callable[..., Outcome], git_repo: GitRepo
) -> None:
    """Gerrit honours Change-Id only as a trailer, so a body line is none."""
    git_repo.commit(
        "Fix: body only", trailers=f"{CHANGE_ID}\n\nClosing prose, not a trailer."
    )
    head = git_repo.head()
    outcome = run_script(SCRIPT, cwd=git_repo.path)
    assert outcome.returncode == EXIT_NO_CHANGE_ID
...
def test_refuses_message_file_with_body_only_change_id(
    run_script: Callable[..., Outcome], gerrit_repo: GitRepo, tmp_path: Path
) -> None:
    """The same holds for a supplied message: the ID must be a trailer."""
    msg = write_message(
        tmp_path / "msg", "Fix: reworded", "", CHANGE_ID, "", "Closing prose."
    )
    head = gerrit_repo.head()
...
def test_refuses_message_file_with_extra_change_id(
    run_script: Callable[..., Outcome], gerrit_repo: GitRepo, tmp_path: Path
) -> None:
    """HEAD's ID plus a second one is an invalid trailer state: refuse."""
    msg = write_message(tmp_path / "msg", "Fix: two ids", "", CHANGE_ID, OTHER_ID)
    head = gerrit_repo.head()
    outcome = run_script(SCRIPT, str(msg), cwd=gerrit_repo.path)
    assert outcome.returncode == EXIT_FILE_LACKS_ID
    assert gerrit_repo.head() == head
