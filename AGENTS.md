...
| Subject case         | Type and description capitalized (§6.6)           |
...
| Approving/merging    | Never self-approve or merge (§10.4)               |
| Stale PR branch      | "Update branch", never force push (§10.4)         |
...
`lfreleng-actions` repository MUST follow this document, whether run by
an organisation maintainer, from a fork, or by a third-party
contributor. A contribution MUST be judged against these rules whether
or not the agent that produced it loaded this file; §12 describes how
repositories point agents here, and that discovery is a convenience
rather than the source of authority.

**Precedence.** The list below ranks *contribution guidance* only. An
agent host's own system and safety instructions sit above all of it,
and nothing here asks an agent to set those aside. Within that limit,
apply these in order, highest authority first:
...
4. Contributor-local or operator instructions: personal `AGENTS.md`,
   `CLAUDE.md`, IDE rules, fork-level configuration.
5. Model defaults.
...
- Agents MUST NOT create or modify code through the GitHub APIs. Work
  locally and push.
- **Every** commit on a branch MUST be signed, merge commits
  included: one unsigned commit makes the whole PR unmergeable.
...
| Capitalize the type and the description       | gitlint CT1 and T7                     |
| Do not end the subject line with a period     | gitlint T3                             |
| Use the imperative mood in the subject line   | Manual                                 |
| Wrap the body at 72 characters                | gitlint B1, where `.gitlint` sets it   |
...
URLs, and an over-long `Signed-off-by` or `Co-authored-by` trailer.
...
`Fix(calendar): Correct race condition` are both valid. Type and
description both start with a capital. Allowed types, enforced by
gitlint:
...
Each repository SHOULD carry the `.gitlint` that
[`actions-template`](https://github.com/lfreleng-actions/actions-template)
holds, which is the canonical copy; repositories generated from the
template inherit it.

```ini
[general]
# gitlint is switching its regex options to 'search' semantics; opting
# in now also silences the deprecation warning.
...
# CT1 validates the type and stops at the colon, so this requires a
# capital in the description too. The leading anchor is required: the
# rule calls search(), so an unanchored pattern never fails. The
...
# A DCO trailer cannot be wrapped and CC1 makes one mandatory, so B1 is
# waived for a body carrying an over-long one. [ignore-body-lines]
# cannot serve here: it strips the line before CC1 sees it, and the
# sign-off check then fails instead.
[ignore-by-body]
...
what an agent can verify locally. Write new commits to the target. Agents
MUST NOT rewrite existing history, or reject another contributor's
commit, against a limit the repository has not configured.
...
| Situation                          | Command                                                                                                              |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Content changed, message unchanged | `git commit -S -s --amend --no-edit`                                                                                 |
| Message edited by hand             | `git commit -S -s --amend` (the editor opens with the Change-Id intact)                                              |
| Message supplied programmatically  | Write the **full** message, including the original `Change-Id` line, to a file; `git commit -S -s --amend -F <file>` |
...
Agents MUST NOT use `--no-verify`, nor `pre-commit.ci autofix`: its
commits are unsigned and block the merge (§5). Apply the fix locally
and commit it signed.
...
```bash
scripts/agents/aislop-gate.sh <pr-base-ref>   # working tree vs merge base
scripts/agents/aislop-gate.sh --staged        # exactly what is staged
scripts/agents/aislop-gate.sh --changes       # working tree vs HEAD
```
...
### 10.4 Reviews and merging

A PR requires an approving review from another member of the
organisation. An agent MUST NOT offer to approve or merge a PR raised
under the contributor's own identity: GitHub blocks self-approval, and
`require_last_push_approval` stops the last pusher supplying it either.
Report the PR as ready and name what it awaits.

To refresh a stale branch, prefer GitHub's **Update branch**: the merge
it creates is signed and existing approvals survive. A content force
push dismisses them, costing another review round.

## 11. Copilot Review Cycles

Not applicable to Gerrit-backed repositories (§3). Copilot reviews
trigger automatically on every push; request one manually (§11.4) only
when a push produced none. Where both `origin` and
...
git commit -S -s --amend          # at each edit stop
```
...
1. **SPDX copyright holder.** The organisation has published no policy.
...
2. **A platform-level channel for this policy.** The §12 stub is the
...
- **`aislop --changes` and `--base` read the working tree, untracked
  files included.** Verified against 0.16.0: a new file is scored
  before it is staged. The gate adds `git ls-files --others` to its own
  file list for those modes, or its language check would speak for
  files aislop scanned but it never saw.
